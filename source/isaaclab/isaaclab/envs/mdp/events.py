...
class _RandomizeRigidBodyMaterialPhysx:
    ...
    def __init__(
        ...
        if isinstance(asset, BaseArticulation) and asset_cfg.body_ids != slice(None):
            # Newton backend doesn't have link_paths or max_shapes — skip per-body indexing
            if hasattr(asset.root_view, "link_paths") and hasattr(asset.root_view, "max_shapes"):
...
class randomize_joint_parameters(ManagerTermBase):
    ...
    def __init__(self, cfg: EventTermCfg, env: ManagerBasedEnv):
        ...
        if self._backend == "physx":
            self.default_dynamic_joint_friction_coeff = wp.to_torch(
                self.asset.data.joint_dynamic_friction_coeff
            ).clone()
            self.default_viscous_joint_friction_coeff = wp.to_torch(
                self.asset.data.joint_viscous_friction_coeff
            ).clone()
